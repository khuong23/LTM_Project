                ...
                <div class="problem-layout">
                    ...
                    <section class="left-panel">
                        <div class="card">
                            ...
                            <form action="${pageContext.request.contextPath}/SubmitCode" method="post"
                                class="submit-form">
...
<c:if test="${not empty lastSubmission}">${lastSubmission.code}</c:if>
                    ...
                    <section class="right-panel">
                        ...
                        <div class="card">
                            ...
                            <button type="button" class="btn btn-secondary" onclick="toggleHistory()"
                                style="width: 100%; margin-bottom: 10px;">Xem lịch sử nộp bài</button>

                            <div id="history-container" style="display: none;">
                                <table class="history-table"
                                    style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                                    <thead>
                                        <tr style="background-color: #1e2a3a; color: #00aaff;">
                                            <th style="padding: 8px; border: 1px solid #333;">ID</th>
                                            <th style="padding: 8px; border: 1px solid #333;">Trạng thái</th>
                                            <th style="padding: 8px; border: 1px solid #333;">Điểm</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <c:forEach items="${submissionHistory}" var="sub">
                                            <tr
                                                style="background-color: #2a3a54; border-bottom: 1px solid #333; color: #ffffff;">
                                                <td style="padding: 8px; text-align: center; border: 1px solid #333;">
                                                    ${sub.submit_id}</td>
                                                <td style="padding: 8px; text-align: center; border: 1px solid #333;"
                                                    class="status-${sub.status}">${sub.status}</td>
                                                <td style="padding: 8px; text-align: center; border: 1px solid #333;">
                                                    ${sub.score}</td>
                                            </tr>
                                        </c:forEach>
                                        <c:if test="${empty submissionHistory}">
                                            <tr>
                                                <td colspan="3"
                                                    style="padding: 8px; text-align: center; border: 1px solid #333; color: #ffffff;">
                                                    Chưa có lịch sử nộp bài.</td>
                                            </tr>
                                        </c:if>
                                    </tbody>
                                </table>
